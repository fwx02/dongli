    ...
    try:
        ...
        for i, book in enumerate(books):
            ...
            try:
                # 尝试插入，如果存在则更新
                cursor.execute(
                    """INSERT OR REPLACE INTO books (title, publish_month, first_seen, last_seen, is_published, crawl_count) 
                       VALUES (?, ?, COALESCE((SELECT first_seen FROM books WHERE title = ?), ?), ?, (COALESCE((SELECT crawl_count FROM books WHERE title = ?), 0) + 1), 0)""",
                    (title, book["publish_month"], title, book["first_seen"], book["last_seen"], title)
                )
                ...
                if cursor.rowcount > 0:
                    if cursor.lastrowid:
                        ...
                        logging.info(f"📚 书籍已存在，更新爬取次数: '{title}'")
            except Exception as e:
...
def main():
    ...
    try:
        ...
        # 批量添加新书到数据库
        new_count = batch_add_books(new_books)
        ...
        message_content = ""
        if new_count > 0:
            message_content += f"🎉 发现 {new_count} 本新书:\n"
            for book in new_books:
                message_content += f"- {book['title']} ({book['publish_month']})\n"
        if published_books:
            message_content += f"📚 {len(published_books)} 本书已出版:\n"
            for book in published_books:
                message_content += f"- {book['title']} ({book['publish_month']})\n"
        
        if message_content:
            send_combined_message("书籍更新通知", message_content)
