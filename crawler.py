...
def batch_add_books(books):
    ...
    try:
        ...
        for i, book in enumerate(books):
            title = book["title"]  # 直接使用原始标题，不进行规范化
            logging.debug(f"处理书籍 #{i + 1}/{len(books)}: '{title}'")

            try:
                # 尝试插入，如果存在则更新last_seen和crawl_count
                cursor.execute(
                    """INSERT INTO books (title, publish_month, first_seen, last_seen, is_published, crawl_count) 
                       VALUES (?, ?, ?, ?, 0, 1)
                       ON CONFLICT(title) DO UPDATE 
                       SET last_seen = excluded.last_seen, 
                           crawl_count = crawl_count + 1""",
                    (title, book["publish_month"], book["first_seen"], book["last_seen"])
                )

                if cursor.rowcount > 0:
                    # 获取当前行的crawl_count值
                    cursor.execute("SELECT crawl_count FROM books WHERE title = ?", (title,))
                    count = cursor.fetchone()[0]
                    
                    if count == 1:
                        new_count += 1
                        logging.info(f"✅ 新书入库: '{title}' ({book['publish_month']})")
                    else:
                        updated_count += 1
                        logging.info(f"📊 书籍更新: '{title}', 已爬取 {count} 次")
                else:
                    logging.info(f"📚 书籍已存在: '{title}'")
            except Exception as e:
                logging.error(f"❌ 添加书籍失败: '{title}', 错误: {str(e)}")
        ...
        conn.commit()
        conn.close()
        logging.info(f"✅ 共添加 {new_count} 本新书，更新 {updated_count} 本已有书籍")
...
def check_and_mark_published_books(current_books):
    """检查并标记已出书的书籍"""
    try:
        current_titles = [book["title"] for book in current_books]  # 使用原始标题
        unpublished_books = get_unpublished_books()

        published_books = []
        for book in unpublished_books:
            if book["title"] not in current_titles:  # 直接比较原始标题
...
def get_unpublished_books():
    ...
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute("SELECT title, publish_month, first_seen, last_seen, crawl_count FROM books WHERE is_published = 0")
        ...
        for page in range(1, 4):
            ...
            for book in page_books:
                ...
                if not exists:
                    ...
                    logging.info(f"🔍 发现新书: {title} ({publish_month})")
        ...
        if books_by_month and WECHAT_WORK_WEBHOOK:
            ...
            for month in sorted_months:
                ...
                for i, book in enumerate(books_sorted, 1):
                    ...
                    waiting_message += f"{i}. **{book['title']}** (等待{days_waiting}天, 爬取{book['crawl_count']}次)\n"
